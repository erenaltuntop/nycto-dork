...
logfile = open("Nycto-check.txt", "a")
lfi_log_file = open("Nycto-dOrk-lfi.txt", "a")

threads = []
finallist = []
vuln = []

header = ['Mozilla/5.0 (X11; Linux x86_64; rv:45.0) Gecko/20100101 Firefox/45.0']

domains = {
    'Default': ['com','fr','net','edu','gov','info'],
    # Diğer domain gruplarını ekleyebilirsiniz
}

def logo():
    print(G + "\n                                                                   ")
    print(G + "Nycto - Dork - Scanner\n")
...
def get_sitearray():
    ...
    sitekey = input("\nChoose your target (if you don't know choose default): ")
    try:
        sitekey_int = int(sitekey)
        if 1 <= sitekey_int <= len(keys):
            return domains[keys[sitekey_int - 1]]
        else:
            print(R + "Invalid choice, using Default.")
            return domains['Default']
    except:
        print(R + "Invalid input, using Default.")
        return domains['Default']

sitearray = get_sitearray()

inurl = input('\nEnter your dork (without "inurl"): ')
numthreads = int(input('Enter no. of threads: '))
maxc = int(input('Enter no. of pages: '))

print(f"\nNumber of headers: {len(header)}")
...
def search(inurl, maxc):
    ...
    for site in sitearray:
        ...
        try:
            while page < maxc:
                ...
                text = opener_web.open(request_web).read().decode('utf-8', errors='ignore')
                stringreg = re.compile('(?<=href=")(.*?)(?=")')
                names = stringreg.findall(text)
                page += 1
                for name in names:
                    if name not in urls:
                        if re.search(r'\(', name) or re.search("<", name) or re.search(r"^/", name) or re.search(r"^(http://)\d", name):
                            continue
                        if any(x in name for x in ["google", "duckduckgo", "ixquick", "webcrawler", "dogpile", "yippy", "Bing", "youtube", "phpbuddy", "iranhack", "phpbuilder", "codingforums", "phpfreaks", "br.search.yahoo", "ajax.googleapis", "search.lycos", "gigablast", "web.search.naver", "dmoz", "%"]):
                            continue
                        urls.append(name)
                percent = int((page / maxc) * 100)
                sys.stdout.write(f"\rSite: {site} | Nycto Collected urls: {len(urls)} | Percent Done: {percent}% | Current page no.: {page} <> ")
                sys.stdout.flush()
        except KeyboardInterrupt:
            print("\nInterrupted by user.")
            break
    ...
    for url in urls:
        try:
            ...
            pass
    print(f"[+] URLS (sorted): {len(finallist)}")
    return finallist

usearch = search(inurl, maxc)

# Thread sınıfları ve test fonksiyonlarını Python3 uyumlu şekilde yazmanız gerekiyor.
# Örnek menü döngüsü:
...
while True:
    ...
    choice = input(": ")
    if choice == '1':
        print("SQLi Testing seçildi. Fonksiyonları ekleyin.")
        # injtest() çağrılabilir
    elif choice == '2':
        print("LFI Testing seçildi. Fonksiyonları ekleyin.")
        # lfitest() çağrılabilir
    elif choice == '3':
        print("SQLi ve LFI Testing seçildi. Fonksiyonları ekleyin.")
        # injtest() ve lfitest() çağrılabilir
    elif choice == '4':
        ...
        print(G + f"\nVulnerabilities found: {len(vuln)}")
    elif choice == '0':
        print(R + "\nExiting...")
        sys.exit(0)
    else:
        print(R + "Invalid choice, try again.")(1)
